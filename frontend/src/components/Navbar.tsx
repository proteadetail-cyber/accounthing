    ...
    <header className="sticky top-0 z-40 w-full bg-[#EBE7DF] border-b border-slate-300/80 shadow-sm">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-heading tracking-wider uppercase transition-all flex items-center gap-1.5 shadow-sm active:scale-95 whitespace-nowrap ${
                isCalculatorOpen || isNotepadOpen
                  ? isPaper1
                    ? 'bg-cyan-300 text-cyan-950 shadow-cyan-600/20'
                    : 'bg-rose-300 text-rose-950 shadow-rose-600/20'
                  : isPaper1
                    ? 'bg-cyan-300 text-cyan-950 hover:bg-cyan-400'
                    : 'bg-rose-300 text-rose-950 hover:bg-rose-400'
              }`}
              title="Open Utility Tools"
            >
              <Wrench className="w-3.5 h-3.5 shrink-0" style={{ color: isCalculatorOpen || isNotepadOpen ? '#FFFFFF' : isPaper1 ? '#0369A1' : '#BE123C' }} />
              <span>TOOLS</span>
          ...
          <button
            ...
            className={`px-2 py-1.5 sm:py-2 rounded-xl text-xs font-mono font-bold transition-all flex items-center gap-1 shrink-0 shadow-sm ${
              isPaper1
                ? 'bg-cyan-300 text-cyan-950 hover:bg-cyan-400'
                : 'bg-rose-300 text-rose-950 hover:bg-rose-400'
            }`}
          ...
          >
            <Globe className="w-3.5 h-3.5 shrink-0" style={{ color: isPaper1 ? '#0369A1' : '#BE123C' }} />
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-slate-300 bg-[#FFF2A8] p-6 text-center text-slate-950 shadow-2xl">
            <LogOut className="mx-auto mb-3 h-8 w-8 text-rose-700" />
            <h2 className="font-heading text-lg tracking-wide">
              {language === 'af' ? 'Verlaat rekening?' : 'Exit account?'}
            </h2>
            <p className="mt-2 text-sm font-sans text-slate-700">
              {language === 'af' ? 'Is jy seker jy wil uitteken?' : 'Are you sure you want to log out?'}
            </p>
            <div className="mt-5 flex gap-3">
              <button
                type="button"
                onClick={() => setShowLogoutConfirm(false)}
                className="flex-1 rounded-xl border border-slate-400 bg-white/70 px-4 py-2.5 font-sans text-xs font-bold uppercase text-slate-800 transition-colors hover:bg-white"
              >
              ...
              <button
                type="button"
                onClick={logout}
                className="flex-1 rounded-xl bg-rose-700 px-4 py-2.5 font-sans text-xs font-bold uppercase text-white transition-colors hover:bg-rose-800"
              >
                {language === 'af' ? 'VERLAAT' : 'EXIT'}
              </button>
            </div>
          </div>
        </div>
